...
/**
 * @brief Executes adversarial input security tests against the kernel boundary.
 *
 * This test suite injects maliciously crafted and deliberately out-of-bounds pointer 
 * addresses into system calls to evaluate the robustness of the kernel's pointer validation
 * firewall. The firewall strictly separates user memory from kernel memory.
 *
 * Expected behavior:
 * - Pointers mapping to the NULL page or below the user-space barrier are firmly rejected.
 * - Pointers targeting kernel data structures or high-memory spaces are rejected.
 * - The `validate_user_pointer` security primitive explicitly accepts pointers falling 
 *   exactly on the lower and upper bounds of valid user space.
 *
 * Edge cases covered:
 * - Null pointer dereference attempts.
 * - Kernel memory read/write exploits.
 * - High unmapped memory exploits.
 * - Absolute boundary value analysis (testing exact valid edges).
 */
void run_adversarial_tests(void) {
    ...
    // (These prove that Syscall rejects external or sensitive memory addresses)
    // =========================================================================
    ...
    // Validate the exact last byte. The firewall must verify the address is technically legal.
